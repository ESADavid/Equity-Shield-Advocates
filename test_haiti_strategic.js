...
try {
  ...
  console.log(`🏗️  Infrastructure Projects: ${portfolio.infrastructure.length}`);
  ...
  portfolio.debt.forEach(debt => {
    ...
    console.log(`     Face Value: $${(debt.faceValue / 1000000000).toFixed(2)}B`);
    console.log(`     Target Acquisition: $${(debt.targetAcquisitionPrice / 1000000000).toFixed(2)}B`);
    console.log(`     Expected Yield: ${(debt.expectedYield * 100).toFixed(1)}%`);
...
try {
  ...
  console.log(`   - Debt Acquisition: ${summary.investmentBreakdown.debtAcquisition}`);
  console.log(`   - Infrastructure: ${summary.investmentBreakdown.infrastructure}`);
  console.log(`   - AI Centers: ${summary.investmentBreakdown.aiCenters}`);
  console.log(`   - Military: ${summary.investmentBreakdown.military}`);
  console.log(`   - Mineral Development: ${summary.investmentBreakdown.mineralDevelopment}`);
  console.log('');
...
try {
  ...
  console.log(`💰 Total AI Budget: $${(totalBudget / 1000000000).toFixed(2)}B`);
  console.log('');
} catch (error) {
...
try {
  ...
  console.log(`   GPUs:`);
  console.log(`     - NVIDIA H100: ${resources.hardware.gpus.nvidia_h100.toLocaleString()}`);
  console.log(`     - NVIDIA Blackwell: ${resources.hardware.gpus.nvidia_blackwell.toLocaleString()}`);
  console.log(`     - AMD MI300: ${resources.hardware.gpus.amd_mi300.toLocaleString()}`);
  console.log(`     - Total Cost: ${resources.hardware.gpus.totalCost}`);
  console.log(`   CPUs:`);
  console.log(`     - AMD EPYC: ${resources.hardware.cpus.amd_epyc.toLocaleString()}`);
  console.log(`     - Intel Xeon: ${resources.hardware.cpus.intel_xeon.toLocaleString()}`);
  console.log(`     - Total Cost: ${resources.hardware.cpus.totalCost}`);
  console.log(`   Memory: ${resources.hardware.memory.ram} - ${resources.hardware.memory.cost}`);
  console.log(`   Storage: ${resources.hardware.storage.nvme} NVMe, ${resources.hardware.storage.hdd} HDD - ${resources.hardware.storage.cost}`);
  ...
  console.log(`   AI Engineers: ${resources.personnel.aiEngineers.count} - ${resources.personnel.aiEngineers.annualCost}/year`);
  console.log(`   Data Scientists: ${resources.personnel.dataScientists.count} - ${resources.personnel.dataScientists.annualCost}/year`);
  console.log(`   Infrastructure Engineers: ${resources.personnel.infrastructureEngineers.count} - ${resources.personnel.infrastructureEngineers.annualCost}/year`);
  console.log(`   Support Staff: ${resources.personnel.supportStaff.count} - ${resources.personnel.supportStaff.annualCost}/year`);
  console.log(`   Total Personnel: ${resources.personnel.totalPersonnelCost}/year`);
  console.log('');
  console.log(`💰 Total Initial Investment: ${resources.totalInitialInvestment}`);
  console.log(`📊 Total Annual Operating Cost: ${resources.totalAnnualOperatingCost}`);
  console.log('');
} catch (error) {
  ...
  portfolio.military.forEach(asset => {
    ...
    if (asset.vessels) {
      ...
      Object.entries(asset.vessels).forEach(([type, data]) => {
        console.log(`       - ${type}: ${data.count} units ($${(data.cost / 1000000).toFixed(0)}M)`);
      });
    }
    if (asset.equipment) {
      console.log(`     Equipment:`);
      Object.entries(asset.equipment).forEach(([type, data]) => {
        console.log(`       - ${type}: ${data.count || 'N/A'} units ($${(data.cost / 1000000).toFixed(0)}M)`);
      });
    }
    if (asset.aircraft) {
    ...
    if (asset.budget) {
      console.log(`     Initial Budget: $${(asset.budget.initial / 1000000000).toFixed(2)}B`);
      console.log(`     Annual Budget: $${(asset.budget.annual / 1000000).toFixed(0)}M`);
    }
...
try {
  ...
  portfolio.minerals.forEach(resource => {
    ...
    console.log(`     Estimated Value: $${(resource.estimatedValue / 1000000000).toFixed(1)}B`);
    ...
    console.log(`     Annual Revenue Projection: $${(resource.annualRevenueProjection / 1000000000).toFixed(2)}B`);
    totalValue += resource.estimatedValue;
    totalAnnualRevenue += resource.annualRevenueProjection;
  });
  console.log('');
  console.log(`📊 Total Estimated Value: $${(totalValue / 1000000000).toFixed(1)}B`);
  console.log(`💰 Total Annual Revenue (at maturity): $${(totalAnnualRevenue / 1000000000).toFixed(2)}B`);
  console.log('');
} catch (error) {
...
try {
  ...
  portfolio.partners.forEach(partner => {
    ...
    partner.agreements.forEach(agreement => console.log(`       - ${agreement}`));
...
try {
  ...
  console.log(`   Face Value: $${(result.debt.faceValue / 1000000000).toFixed(2)}B`);
  console.log(`   Acquired Value: $${(result.debt.acquiredValue / 1000000000).toFixed(2)}B`);
  console.log(`   Discount: ${result.debt.discount}`);
  console.log(`   Expected Yield: ${(result.debt.expectedYield * 100).toFixed(1)}%`);
  console.log('');
...
try {
  ...
  console.log(`   - Portfolio data: ${Object.keys(exportData.portfolio).length} sections`);
