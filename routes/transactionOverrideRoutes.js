...
import {
  authorizeOverride,
  auditOverride,
} from '../middleware/authOverride.js';

// GET /api/transactions/overrides - List all override requests
router.get(
  '/overrides',
  authorizeOverride(['admin', 'override_manager']),
  (req, res) => {
    try {
...
router.put('/:id/override', authorizeOverride(['admin']), (req, res) => {
  try {
    ...
    logger.info(`Updating transaction ${id} with override:`, {
      newValue,
      reason,
    });
  ...
  async (req, res) => {
    try {
      ...
      const { getBlockchainService } =
        await import('../blockchain/blockchainService.js');
