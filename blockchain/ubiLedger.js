...
class UBILedger {
  ...
  async recordPayment(payment) {
    try {
      ...
      info(`UBI payment recorded on blockchain: ${hash} for payment ${payment._id}`);
      return hash;
    } catch (err) {
  ...
  async recordPaymentsBatch(payments) {
    try {
      ...
      info(`UBI payment batch recorded on blockchain: ${hashes.length} blocks created`);
      return hashes;
    } catch (err) {
  ...
  async verifyPayment(paymentId) {
    try {
      ...
      const isValid = await blockchainService.verifyBlock(payment.blockchainHash);
      ...
      return {
        ...
        amount: payment.amount,
        paymentDate: payment.paymentDate,
        reason: isValid ? 'Payment verified on blockchain' : 'Payment verification failed'
      };
    } catch (err) {
  ...
  async getCitizenBlockchainHistory(citizenId, limit = 50) {
    try {
      ...
      for (const payment of payments) {
        try {
          const blockData = await blockchainService.getBlock(payment.blockchainHash);
          if (blockData) {
            history.push({
          ...
          warn(`Failed to get block data for payment ${payment._id}:`, blockErr.message);
        }
      }
  ...
  async auditPayments(startDate, endDate) {
    try {
      ...
      const auditResult = {
        ...
        summary: results,
        integrityScore: results.totalPayments > 0 ?
          ((results.blockchainVerified / results.totalPayments) * 100).toFixed(2) + '%' : '0%'
      };

      info(`UBI payment audit completed: ${results.blockchainVerified}/${results.totalPayments} verified`);
      return auditResult;
    } catch (err) {
  ...
  async processPendingBlockchainRecords() {
    try {
      ...
      info(`Processing ${pendingPayments.length} pending UBI payments for blockchain recording`);
  ...
  async getLedgerStats() {
    try {
      ...
      return {
        ...
        blockchainRecorded,
        pendingRecording,
        recordingCoverage: totalPayments > 0 ?
          ((blockchainRecorded / totalPayments) * 100).toFixed(2) + '%' : '0%',
