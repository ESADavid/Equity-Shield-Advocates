  ...
  if (!value || typeof value !== 'string' || value.trim().length === 0) {
    errors.push(
      new PayrollValidationError(
        fieldName,
        ''.concat(fieldName, ' is required and must be a non-empty string')
      )
    );
  }
}
  ...
  if (typeof value !== 'number' || value < min || value > max) {
    errors.push(
      new PayrollValidationError(
        fieldName,
        ''
          .concat(fieldName, ' must be between ')
          .concat(min, ' and ')
          .concat(max),
        value
      )
...
function validateOptionalPayrollFields(input, errors) {
  validateOptionalNumericField(
    ...
    'hoursWorked',
    0,
    payroll_js_1.PAYROLL_CONSTANTS.MAX_HOURS_WORKED,
    errors
  );
  validateOptionalNumericField(
    input.hourlyRate,
    'hourlyRate',
    0,
    payroll_js_1.PAYROLL_CONSTANTS.MAX_HOURLY_RATE,
    errors
  );
  validateOptionalNumericField(
    input.overtimeHours,
    'overtimeHours',
    0,
    payroll_js_1.PAYROLL_CONSTANTS.MAX_OVERTIME_HOURS,
    errors
  );
  validateOptionalNumericField(
    input.taxRate,
    'taxRate',
    payroll_js_1.PAYROLL_CONSTANTS.MIN_TAX_RATE,
    payroll_js_1.PAYROLL_CONSTANTS.MAX_TAX_RATE,
    errors
  );
  validateOptionalNumericField(
    input.deductions,
    'deductions',
    0,
    payroll_js_1.PAYROLL_CONSTANTS.MAX_DEDUCTIONS,
    errors
  );
  validateOptionalNumericField(
    input.bonuses,
    'bonuses',
    0,
    payroll_js_1.PAYROLL_CONSTANTS.MAX_BONUSES,
    errors
  );
}
/**
...
function sanitizeEmployeeInput(input) {
  ...
  var accountNumber =
    (_b = input.accountNumber) === null || _b === void 0
      ? void 0
      : _b.replace(/\D/g, '');
  if (accountNumber) {
  ...
  var routingNumber =
    (_c = input.routingNumber) === null || _c === void 0
      ? void 0
      : _c.replace(/\D/g, '');
  if (routingNumber) {
