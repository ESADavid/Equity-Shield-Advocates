...
const config = {
  ...
  targetPath: (process.argv.find(arg => arg.startsWith('--path=')) || '--path=.').split('=')[1],
  excludeDirs: [
    'node_modules',
    '.git',
    'dist',
...
function replaceConsoleInFile(filePath, content) {
  ...
  if (!newContent.includes('import') && !newContent.includes('from \'../utils/loggerWrapper.js\'')) {
...
function processFile(filePath) {
  ...
  console.log(`\n📄 ${relativePath}`);
  console.log(`   Type: ${isTest ? '🧪 TEST FILE (keep console)' : '⚠️  PRODUCTION FILE (needs replacement)'}`);
  ...
  if (!isTest) {
    statements.forEach((stmt, idx) => {
      ...
      console.log(`   ${idx + 1}. Line ${lineNumber}: ${stmt.statement} → ${suggestion}`);
    });
    
    if (!config.dryRun) {
      const { newContent, replacementCount } = replaceConsoleInFile(filePath, content);
...
function main() {
  ...
  console.log(`Mode: ${config.dryRun ? 'DRY RUN (no changes)' : 'REPLACE MODE'}`);
