...
class ProductionScaler {
  ...
  async run() {
    try {
      this.log(
        `Starting scaling to ${this.targetScale.toLocaleString()} citizens...`,
        'step'
      );
  ...
  async calculateResourceRequirements() {
    ...
    this.log(
      `Target citizens: ${requirements.citizens.toLocaleString()}`,
      'metric'
    );
    this.log(
      `Concurrent users: ${requirements.estimatedConcurrentUsers.toLocaleString()}`,
      'metric'
    );
  ...
  async scaleInfrastructure() {
    ...
    const scalingSteps = [
      ...
      },
      {
        ...
        command:
          'kubectl apply -f k8s/resource-quotas-scaled.yml -n oscar-broome-production',
        skipOnError: true,
      },
      {
  ...
  async scaleDatabases() {
    ...
    const dbScaling = [
      ...
      },
      {
        ...
        command:
          'kubectl scale deployment redis --replicas=3 -n oscar-broome-production',
      },
      {
        name: 'Update database connection pools',
        command:
          'kubectl set env deployment/oscar-broome-app DB_POOL_SIZE=100 -n oscar-broome-production',
      },
    ];
  ...
  async scaleApplications() {
    ...
    const appScaling = [
      ...
      },
      {
        ...
        command:
          'kubectl set resources deployment oscar-broome-app --limits=cpu=2,memory=4Gi --requests=cpu=1,memory=2Gi -n oscar-broome-production',
      },
    ];
    ...
    const lbUpdates = [
      {
        ...
        command:
          'kubectl apply -f k8s/load-balancer-scaled.yml -n oscar-broome-production',
        skipOnError: true,
      },
      {
        name: 'Configure connection limits',
        command:
          'kubectl annotate service oscar-broome-service service.beta.kubernetes.io/aws-load-balancer-connection-idle-timeout=3600 -n oscar-broome-production',
        skipOnError: true,
      },
    ];
    ...
    const validations = [
      {
        ...
        command:
          'kubectl get pods -n oscar-broome-production | grep Running | wc -l',
      },
      {
    ...
    for (const test of loadTests) {
      ...
      if (!this.dryRun) {
        try {
          // Simulate load test
          this.log(
            `Simulating ${test.target.toLocaleString()} users for ${test.duration}s`,
            'info'
          );
    ...
    const optimizations = [
      {
        name: 'Enable caching',
        command:
          'kubectl set env deployment/oscar-broome-app ENABLE_CACHE=true CACHE_TTL=600 -n oscar-broome-production',
      },
      {
        name: 'Configure CDN',
        command:
          'kubectl apply -f k8s/cdn-config.yml -n oscar-broome-production',
        skipOnError: true,
      },
      {
        name: 'Optimize database queries',
        command:
          'kubectl exec -n oscar-broome-production mongodb-0 -- mongo --eval "db.adminCommand({setParameter: 1, internalQueryExecMaxBlockingSortBytes: 335544320})"',
        skipOnError: true,
      },
    ];
  ...
  async generateScalingReport() {
    ...
    if (!this.dryRun) {
      fs.writeFileSync('scaling-report.json', JSON.stringify(report, null, 2));
  ...
  showSummary() {
    ...
    if (this.requirements) {
      console.log(
        `   - Target citizens: ${this.requirements.citizens.toLocaleString()}`
      );
      console.log(`   - Pods deployed: ${this.requirements.requiredPods}`);
      console.log(
        `   - Database nodes: ${this.requirements.requiredDatabaseNodes}`
      );
      console.log(
        `   - Memory allocated: ${this.requirements.requiredMemoryGB}GB`
      );
    ...
    console.log(
      '   2. Check metrics: kubectl top pods -n oscar-broome-production'
    );
