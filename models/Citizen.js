...
const CitizenSchema = new mongoose.Schema({
  ...
  educationStatus: {
    ...
    complianceStatus: {
      type: String,
      enum: ['compliant', 'in_progress', 'non_compliant', 'grace_period', 'exempt'],
      default: 'in_progress'
    },
  ...
  employmentInfo: {
    ...
    sector: {
      type: String,
      enum: ['military', 'law', 'technology', 'agriculture', 'government', 'private', 'self_employed', 'other']
    },
  ...
  metadata: {
    registrationSource: {
      type: String,
      enum: ['online', 'in_person', 'mobile_app', 'government_office', 'community_center'],
...
CitizenSchema.index({ 'personalInfo.nationalId': 1 });
...
CitizenSchema.methods.encryptBankingInfo = function(accountNumber, routingNumber) {
  const algorithm = 'aes-256-cbc';
  const key = process.env.ENCRYPTION_KEY || crypto.randomBytes(32);
  const iv = crypto.randomBytes(16);
  
  const cipherAccount = crypto.createCipheriv(algorithm, key, iv);
  let encryptedAccount = cipherAccount.update(accountNumber, 'utf8', 'hex');
  encryptedAccount += cipherAccount.final('hex');
...
CitizenSchema.methods.checkUBIEligibility = function() {
  ...
  if (!this.verification.identityVerified || !this.verification.bankingVerified) {
