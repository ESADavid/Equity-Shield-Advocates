...
courseSchema.virtual('completionRate').get(function() {
  ...
  const completed = this.enrolledStudents.filter(s => s.status === 'completed').length;
...
courseSchema.methods.updateStudentProgress = function(studentId, progress, additionalData = {}) {
  ...
  info(`Updated progress for student ${studentId} in course ${this._id}: ${progress}%`);
  return enrollment;
};

// Method to complete course for student
courseSchema.methods.completeCourse = function(studentId, grade = null, certificateId = null) {
  const enrollment = this.enrolledStudents.find(
    s => s.studentId.toString() === studentId.toString()
  );
...
courseSchema.methods.updateStatistics = function() {
  ...
  if (enrollments.length > 0) {
    this.statistics.completionRate = Math.round((completed.length / enrollments.length) * 100);

    const totalProgress = enrollments.reduce((sum, e) => sum + (e.progress || 0), 0);
    this.statistics.averageProgress = Math.round(totalProgress / enrollments.length);
  }
...
courseSchema.statics.findByCategoryAndDifficulty = function(category, difficulty, limit = 20) {
  return this.find({
    category,
    difficulty,
    status: 'published'
...
courseSchema.statics.getCourseStats = async function() {
  ...
  result.byCategory = Object.entries(categoryStats).map(([category, stats]) => ({
    category,
    ...stats
  }));
...
courseSchema.pre('save', function(next) {
  ...
  this.totalDuration = this.curriculum.reduce((total, module) => total + (module.duration || 0), 0);
