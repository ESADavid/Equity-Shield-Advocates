...
debtSchema.statics = {
  ...
  getPortfolioAnalytics: async function (tenantId) {
    ...
    const totalAcquiredValue = debts.reduce(
      (sum, debt) => sum + Number.parseFloat(debt.acquiredValue.toString()),
      0
    );

    const totalCurrentValue = debts.reduce(
      (sum, debt) => sum + Number.parseFloat(debt.currentValue.toString()),
      0
    );
    ...
    const weightedYield = debts.reduce((sum, debt) => {
      const weight =
        Number.parseFloat(debt.acquiredValue.toString()) / totalAcquiredValue;
    ...
    for (const debt of debts) {
      ...
      geographicDistribution[debt.country].value += Number.parseFloat(
        debt.acquiredValue.toString()
      );
    ...
    for (const debt of debts) {
      ...
      entityTypeDistribution[debt.entityType].value += Number.parseFloat(
        debt.acquiredValue.toString()
      );
