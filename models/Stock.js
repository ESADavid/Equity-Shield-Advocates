...
const StockSchema = new mongoose.Schema({
  ...
  avgPurchasePrice: { type: mongoose.Decimal128, required: true },
  currentPrice: { type: mongoose.Decimal128, default: 0 },
...
StockSchema.statics.generateStockId = async function() {
  let id;
  do {
    id = 'STK-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6).toUpperCase();
