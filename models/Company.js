...
const CompanySchema = new mongoose.Schema({
  ...
  sharePrice: { type: mongoose.Decimal128, default: 0 },
  totalValue: { type: mongoose.Decimal128, default: 0 },
...
CompanySchema.statics.generateCompanyId = async function() {
  let id;
  do {
    id = 'COMP-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6).toUpperCase();
  } while (await this.findOne({ companyId: id }));
  return id;
};

CompanySchema.pre('save', async function(next) {
  if (!this.companyId) this.companyId = await this.constructor.generateCompanyId();
  next();
});
