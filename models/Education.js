...
EducationProgramSchema.statics.generateProgramId = async function(programType) {
  let programId;
  let exists = true;
...
EducationProgramSchema.methods.updateCitizenProgress = function(citizenId, progressData) {
...
EducationProgramSchema.methods.calculateStatistics = function() {
  const activeCitizens = this.enrolledCitizens.filter(ec => ec.status === 'active');
  const completedCitizens = this.enrolledCitizens.filter(ec => ec.status === 'completed');
  ...
  const totalProgress = this.enrolledCitizens.reduce((sum, ec) => sum + ec.progress, 0);
  const averageCompletionRate = totalProgress / this.enrolledCitizens.length;

  // Average grade
  const allGrades = completedCitizens.flatMap(ec => ec.grades.map(g => g.score));
