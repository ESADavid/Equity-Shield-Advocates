...
async function testEndpoint(name, method, path, expectedStatus = 200) {
  ...
  try {
    ...
    if (response.statusCode === expectedStatus) {
      results.passed.push(`✓ ${name}: ${method} ${path} (${response.statusCode})`);
      return true;
    } else {
      results.failed.push(`✗ ${name}: Expected ${expectedStatus}, got ${response.statusCode}`);
      return false;
    }
  } catch (error) {
...
async function runTests() {
  ...
  await testEndpoint('Citizen Portal Health', 'GET', '/api/citizen-portal/health');
  await testEndpoint('Citizen Statistics', 'GET', '/api/citizen-portal/statistics');
  ...
  await testEndpoint('Notification Health', 'GET', '/api/notifications-v2/health');
  await testEndpoint('Notification Templates', 'GET', '/api/notifications-v2/templates');
  await testEndpoint('Notification Statistics', 'GET', '/api/notifications-v2/statistics');
