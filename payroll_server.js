...
app.use(function (_req, _res, next) {
  ...
  const overrideUser =
    _req.headers['x-override-user'] || _req.query.overrideUser;
...
app.post('/api/payroll/employees', (req, res) => {
  ...
  try {
    ...
    if (existing) {
      const updateResult = payrollSystem_1.payrollSystem.updateEmployee(
        employee.id,
        employee
      );
...
app.post('/api/payroll/process', (_req, res) => {
  ...
  try {
    ...
    logger_js_1.logger.error('Error processing payroll:', {
      error: error.message,
      stack: error.stack,
    });
