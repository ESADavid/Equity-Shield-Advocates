...
export default class ExecutiveDashboard {
  ...
  updateMetrics() {
    ...
    document.getElementById('dailyRevenue').textContent = new Intl.NumberFormat(
      'en-US',
      {
        style: 'currency',
        currency: 'USD',
    ...
    document.getElementById('corporateHomes').textContent =
      new Intl.NumberFormat('en-US').format(
        this.data.purchases?.corporateHomes || 0
      );
  ...
  createFleetChart() {
    ...
    const fleetData = {
      ...
      datasets: [
        {
          data: [5, 3, 2, 4, 1],
          backgroundColor: ['#d4af37', '#1a1a2e', '#16213e', '#0f3460', '#333'],
        },
      ],
    };
  ...
  updateRecentActivity() {
    ...
    const portfolioActivities = [
      {
        type: 'alert',
        message: 'Portfolio volatility increased by 2%',
        date: '2024-01-12',
        amount: 0,
      },
      {
        type: 'recommendation',
        message: 'Consider rebalancing to reduce risk',
        date: '2024-01-11',
        amount: 0,
      },
    ];
    ...
    const activities = [
      {
        type: 'purchase',
        message: 'Added Tesla Model S to fleet',
        date: '2024-01-15',
        amount: 79999,
      },
      {
        type: 'revenue',
        message: 'Daily revenue exceeded $50,000',
        date: '2024-01-14',
        amount: 50000,
      },
      {
        type: 'purchase',
        message: 'Purchased corporate home',
        date: '2024-01-13',
        amount: 2500000,
      },
      ...portfolioActivities,
  ...
  async loadRevenueData() {
    ...
    container.innerHTML = `
            <div class="revenue-streams">
                ${Object.entries(this.data.revenueStreams || {})
                  .map(
                    ([stream, details]) => `
  ...
  async loadAUMData() {
    ...
    assetClassContainer.innerHTML = Object.entries(aum.assetClasses || {})
      .map(
        ([key, val]) => `
  ...
  createAUMChart() {
    ...
    const performanceValues = [
      2400000, 2450000, 2500000, 2480000, 2520000, 2500000,
    ];
  ...
  createAssetPerformanceChart() {
    ...
    const returns = assets.map((asset) =>
      Number.parseFloat(asset.performance?.yearly || 0)
    );
    const volatilities = assets.map((asset) =>
      Number.parseFloat(asset.performance?.volatility || 0)
    );
  ...
  createForecastChart() {
    ...
    const forecastValues = [
      2600000, 2700000, 2800000, 2750000, 2850000, 2900000,
    ];
