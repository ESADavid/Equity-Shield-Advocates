...
ExecutiveDashboard.prototype.loadPremiumOptions = function () {
  ...
  const coverageLevels = Object.keys(
    premiumRates[Object.keys(premiumRates)[0]]
  );
...
ExecutiveDashboard.prototype.loadRecentCalculations = function () {
  const recentCalculations = JSON.parse(
    localStorage.getItem('recentPayrollCalculations') || '[]'
  );
  const container = document.getElementById('recentCalculations');
  if (!container) return;

  container.innerHTML = recentCalculations
    .map(
      (calc) => `
...
ExecutiveDashboard.prototype.calculatePaycheck = function () {
  const hoursWorked =
    Number.parseFloat(document.getElementById('hoursWorked').value) || 0;
  const hourlyRate =
    Number.parseFloat(document.getElementById('hourlyRate').value) || 0;
  const overtimeHours =
    Number.parseFloat(document.getElementById('overtimeHours').value) || 0;
  const taxRate =
    Number.parseFloat(document.getElementById('taxRate').value) || 0;
  const deductionsInput =
    Number.parseFloat(document.getElementById('deductions').value) || 0;
  const bonuses =
    Number.parseFloat(document.getElementById('bonuses').value) || 0;

  const planSelect = document.getElementById('medicalPlanSelect');
  const coverageSelect = document.getElementById('coverageLevelSelect');
  const isExempt = document.getElementById('exemptStatusCheckbox')
    ? document.getElementById('exemptStatusCheckbox').checked
    : false;
