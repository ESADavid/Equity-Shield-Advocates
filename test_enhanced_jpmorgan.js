...
app.use(
  ...
  security.validatePaymentCreation
);
app.use('/api/jpmorgan-payment/refund', security.validateRefund);
app.use(
  '/api/jpmorgan-payment/payment-status/:paymentId',
  security.validatePaymentId
);
app.use(
  '/api/jpmorgan-payment/transactions',
  security.validateTransactionsQuery
);
    ...
    try {
      ...
      if (hasSecurityHeaders) {
        ...
        results.tests.push({
          ...
          status: 'PASS',
          note: 'Headers may vary by environment',
        });
      }
    } catch (error) {
