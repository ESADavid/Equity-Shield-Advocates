...
async function demonstrateQuantumControlCenter() {
  ...
  console.log('Control Center ID:', controlCenter.quantumEngine.getQuantumState('control_center')?.centerId);
  console.log('');
  ...
  try {
    ...
    const reportResult = await controlCenter.executeControlCommand('generate_report', {
      reportType: 'system'
    });
    console.log('✅ Generate Report Result:', JSON.stringify(reportResult, null, 2));
    console.log('');

    // Execute Transaction via Control Center
    console.log('💰 Executing EXECUTE_TRANSACTION command...');
    const transactionResult = await controlCenter.executeControlCommand('execute_transaction', {
      type: 'payment',
      amount: 500.00,
    ...
    console.log('✅ Execute Transaction Result:', JSON.stringify(transactionResult, null, 2));
    console.log('');

    // Update Configuration
    console.log('⚙️ Executing UPDATE_CONFIG command...');
    const configResult = await controlCenter.executeControlCommand('update_config', {
      configKey: 'sync_interval',
      configValue: 15000 // 15 seconds
    });
    console.log('✅ Update Config Result:', JSON.stringify(configResult, null, 2));
    console.log('');

    // Optimize System
    console.log('🔧 Executing OPTIMIZE_SYSTEM command...');
    const optimizeResult = await controlCenter.executeControlCommand('optimize_system');
    console.log('✅ Optimize System Result:', JSON.stringify(optimizeResult, null, 2));
    console.log('');

    // Security Scan
    console.log('🔒 Executing SECURITY_SCAN command...');
    const securityResult = await controlCenter.executeControlCommand('security_scan');
    console.log('✅ Security Scan Result:', JSON.stringify(securityResult, null, 2));
    console.log('');

    // Backup Data
    console.log('💾 Executing BACKUP_DATA command...');
    const backupResult = await controlCenter.executeControlCommand('backup_data');
    console.log('✅ Backup Data Result:', JSON.stringify(backupResult, null, 2));
    console.log('');

    // Scale System
    console.log('📈 Executing SCALE_SYSTEM command...');
    const scaleResult = await controlCenter.executeControlCommand('scale_system', {
      scaleType: 'horizontal',
      scaleFactor: 2
    });
    console.log('✅ Scale System Result:', JSON.stringify(scaleResult, null, 2));
    console.log('');
    ...
    const accountsData = controlCenter.quantumEngine.getQuantumState('jpmorgan_accounts');
    const transactionsData = controlCenter.quantumEngine.getQuantumState('jpmorgan_transactions');
    const paymentsData = controlCenter.quantumEngine.getQuantumState('jpmorgan_payments');
    const reportsData = controlCenter.quantumEngine.getQuantumState('jpmorgan_reports');
    ...
    console.log('Note: Emergency shutdown and restart are demonstrated but not actually executed');
    console.log('In a real scenario, these would require explicit confirmation');
    console.log('');

    // 8. Command History
    console.log('📋 Recent Command History:');
    // In a real implementation, this would show command history
    console.log('Commands executed: sync_data, generate_report, execute_transaction, update_config, optimize_system, security_scan, backup_data, scale_system');
    console.log('');
