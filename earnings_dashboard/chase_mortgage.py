...
CHASE_CONFIG = {
    ...
    'mortgage_api_url': 'https://api.chase.com/mortgage/v1',
    'jwt_secret': os.getenv('JWT_SECRET', 'your_jwt_secret_key'),
    'session_timeout': int(os.getenv('SESSION_TIMEOUT', '3600'))  # 1 hour
}

class ChaseMortgageAPI:
    def __init__(self):
        ...
        self.mortgage_api_url = CHASE_CONFIG['mortgage_api_url']
    ...
    def get_mortgage_accounts(self, account_id=None):
        """Get mortgage account information"""
        ...
        try:
            if account_id:
                url = f"{self.mortgage_api_url}/accounts/{account_id}"
            else:
                url = f"{self.mortgage_api_url}/accounts"
            ...
            return {
                'success': False,
                'error': f'Failed to retrieve mortgage data: {str(e)}'
            }

    def get_mortgage_statement(self, account_id, start_date=None, end_date=None):
        """Get mortgage statement data"""
        if not self._is_token_valid():
            return {'success': False, 'error': 'Authentication required'}
        ...
        try:
            url = f"{self.mortgage_api_url}/accounts/{account_id}/statements"
            ...
            return {
                'success': False,
                'error': f'Failed to retrieve statement data: {str(e)}'
            }
    ...
    def make_payment(self, account_id, amount, payment_date=None):
        ...
        try:
            url = f"{self.mortgage_api_url}/payments"
...
def get_accounts():
    ...
    result = chase_api.get_mortgage_accounts(account_id)
...
def get_statements(account_id):
    ...
    result = chase_api.get_mortgage_statement(account_id, start_date, end_date)
...
def sync_mortgage_data():
    ...
    try:
        # Get all accounts
        accounts_result = chase_api.get_mortgage_accounts()
        ...
        for account in accounts:
            ...
            statements_result = chase_api.get_mortgage_statement(
                account_id,
            ...
            account_data = {
                ...
                'recent_statements': statements_result['data'] if statements_result['success'] else []
            }

            synced_data['accounts'].append(account_data)

        # Save to local file (in production, use database)
        sync_file = os.path.join(os.path.dirname(__file__), 'mortgage_sync_data.json')
        with open(sync_file, 'w') as f:
            json.dump(synced_data, f, indent=2)

        return jsonify({
            'success': True,
            'message': 'Mortgage data synced successfully',
            'data': synced_data
        })
...
def chase_mortgage_page():
    ...
    <html lang="en">
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>🏠 Chase Mortgage Integration</h1>
            <p>Secure access to your mortgage accounts and payment management</p>
          </div>
          ...
          function displayAccounts(accounts) {
            ...
            accountsGrid.innerHTML = accounts.map(account => `
              <div class="account-card">
                <h3>${account.name || 'Mortgage Account'}</h3>
                ...
                <button class="btn" onclick="viewStatements('${account.id}')">View Statements</button>
              </div>
            `).join('');
          }

          // View statements
          async function viewStatements(accountId) {
            try {
              const response = await fetch(`${API_BASE}/statements/${accountId}`, {
                headers: {
                  'Authorization': `Bearer ${authToken}`,
                },
              });
              ...
              if (data.success) {
                alert('Statements loaded successfully! (Check console for details)');
