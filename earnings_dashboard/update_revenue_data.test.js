...
const testDataPath = path_1.default.resolve(
  __dirname,
  '../../owlban_repos/test_revenue.json'
);
...
describe('updateRevenueData', () => {
  beforeAll(async () => {
    ...
    await promises_1.default.mkdir(path_1.default.dirname(testDataPath), {
      recursive: true,
    });
  });
  beforeEach(async () => {
    ...
    await promises_1.default.writeFile(
      testDataPath,
      JSON.stringify(mockRevenueData, null, 2)
    );
  });
  afterEach(async () => {
    // Clean up test file
  ...
  test('should return true when data file exists and is valid', async () => {
    const result = await (0, update_revenue_data_1.default)(
      false,
      testDataPath
    );
  ...
  test('should add sample purchase data when ADD_SAMPLE_DATA is true and incremental is false', async () => {
    ...
    const updatedData = JSON.parse(
      await promises_1.default.readFile(testDataPath, 'utf-8')
    );
