...
try {
  ...
  if (os.platform() === 'win32') {
    // Windows
    execSync(`setx JPMORGAN_CLIENT_ID "${testCredentials.clientId}"`, { stdio: 'inherit' });
    execSync(`setx JPMORGAN_CLIENT_SECRET "${testCredentials.clientSecret}"`, { stdio: 'inherit' });
    execSync(`setx JPMORGAN_MERCHANT_ID "${testCredentials.merchantId}"`, { stdio: 'inherit' });
    execSync(`setx JPMORGAN_TERMINAL_ID "${testCredentials.terminalId}"`, { stdio: 'inherit' });
    execSync(`setx JPMORGAN_ORGANIZATION_ID "${testCredentials.organizationId}"`, { stdio: 'inherit' });
    execSync(`setx JPMORGAN_PROJECT_ID "${testCredentials.projectId}"`, { stdio: 'inherit' });
  } else {
    // Linux/macOS
    execSync(`export JPMORGAN_CLIENT_ID="${testCredentials.clientId}"`, { stdio: 'inherit' });
    execSync(`export JPMORGAN_CLIENT_SECRET="${testCredentials.clientSecret}"`, { stdio: 'inherit' });
    execSync(`export JPMORGAN_MERCHANT_ID="${testCredentials.merchantId}"`, { stdio: 'inherit' });
    execSync(`export JPMORGAN_TERMINAL_ID="${testCredentials.terminalId}"`, { stdio: 'inherit' });
    execSync(`export JPMORGAN_ORGANIZATION_ID="${testCredentials.organizationId}"`, { stdio: 'inherit' });
    execSync(`export JPMORGAN_PROJECT_ID="${testCredentials.projectId}"`, { stdio: 'inherit' });
  }
