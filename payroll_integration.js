...
class PayrollIntegration {
    ...
    async addOrUpdateEmployeePayroll(employee) {
        try {
            ...
            const response = await this.retryRequest(() => axios_1.default.put(url, employee, {
    ...
    async validateDirectDeposit(employee) {
        try {
            if (!employee.accountNumber || !employee.routingNumber) {
            ...
            logger.error('Error validating direct deposit:', error);
