...
class AILearningService {
  ...
  async generateRecommendations(citizenId, currentProgress) {
    try {
      const recommendations = {
        nextCourses: await this.recommendNextCourses(citizenId, currentProgress),
        focusAreas: this.identifyFocusAreas(currentProgress),
        estimatedCompletion: this.estimateCompletion(currentProgress),
        learningPath: await this.generateLearningPath(citizenId, currentProgress)
      };
  ...
  async recommendNextCourses(citizenId, progress) {
    try {
      ...
      const completedCategories = progress.completedCourses?.map(c => c.category) || [];
      
      // Find courses in similar categories
      const recommendations = await Course.find({
        isActive: true,
        category: { $in: completedCategories.length > 0 ? completedCategories : ['technology', 'business'] },
  ...
  identifyFocusAreas(progress) {
    ...
    if (focusAreas.length === 0) {
      focusAreas.push('Problem Solving', 'Critical Thinking', 'Practical Application');
    }
  ...
  async generateLearningPath(citizenId, progress) {
    try {
      ...
      return {
        ...
        level: 'beginner',
        next: ['Introduction to Technology', 'Business Basics', 'Communication Skills'],
        timeline: '3-6 months'
      };
    }
  }
  ...
  identifyImprovements(avgProgress) {
    ...
    if (avgProgress < 50) {
      ...
      improvements.push('Explore advanced topics', 'Share knowledge with others');
    }
  ...
  async generateStudyPlan(citizenId, hoursPerWeek = 5) {
    try {
      ...
      courses.forEach((course, index) => {
        ...
        if (student) {
          const remainingLessons = course.curriculum.length - student.completedLessons.length;
          plan.schedule.push({
            course: course.title,
