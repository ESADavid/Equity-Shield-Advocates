...
const BASE_URL =
  process.env.JPMORGAN_TEST_BASE_URL || 'http://localhost:3000/jpmorgan';
...
class JPMorganTester {
  ...
  async testTreasuryHealth() {
    const response = await axios.get(`${this.baseURL}/treasury/health`, {
      timeout: 10000,
    });
    if (response.data.status !== 'healthy') {
  ...
  async testCashPositions() {
    const response = await axios.get(
      `${this.baseURL}/treasury/cash-positions?currency=USD`
    );
  ...
  async testFXRates() {
    const response = await axios.get(
      `${this.baseURL}/treasury/fx-rates?baseCurrency=USD&quoteCurrency=EUR`
    );
    if (!response.data.success) {
  ...
  async testLiquidityForecast() {
    const response = await axios.get(
      `${this.baseURL}/treasury/liquidity-forecast?days=30&currency=USD`
    );
    if (!response.data.success) {
  ...
  async testPortfolioPerformance() {
    const response = await axios.get(
      `${this.baseURL}/treasury/portfolio-performance?period=1M&currency=USD`
    );
    if (!response.data.success) {
  ...
  async testCashFlowAnalytics() {
    const response = await axios.get(
      `${this.baseURL}/treasury/cash-flow-analytics?granularity=daily&currency=USD`
    );
    if (!response.data.success) {
  ...
  async runAllTests() {
    ...
    await this.runTest('Invalid Payment Creation', () =>
      this.testInvalidPaymentCreation()
    );

    // Treasury endpoints
    await this.runTest('Cash Positions', () => this.testCashPositions());
    await this.runTest('FX Rates', () => this.testFXRates());
    await this.runTest('Liquidity Forecast', () =>
      this.testLiquidityForecast()
    );
    await this.runTest('Portfolio Performance', () =>
      this.testPortfolioPerformance()
    );
    await this.runTest('Cash Flow Analytics', () =>
      this.testCashFlowAnalytics()
    );

    // Transaction endpoints
    await this.runTest('Transaction History', () =>
